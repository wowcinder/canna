//$Id: CollectionElementTest.java 18602 2010-01-21 20:48:59Z hardy.ferentschik $
...
public class CollectionElementTest extends TestCase {
	...
	public void testSimpleElement() throws Exception {
		assertEquals(
				"BoyFavoriteNumbers",
				getCfg().getCollectionMapping(
						Boy.class.getName() + '.' + "favoriteNumbers")
		...
		assertTrue(boy.getFavoriteFood().get("dinner")
				.equals(FavoriteFood.SUSHI));
		assertTrue(boy.getFavoriteFood().get("lunch")
				.equals(FavoriteFood.KUNGPAOCHICKEN));
		assertTrue(boy.getFavoriteFood().get("breakfast")
				.equals(FavoriteFood.PIZZA));
		List result = s
				.createQuery(
	...
	public void testCompositeElement() throws Exception {
		...
		assertEquals("@Parent is failing", boy, boy.getFavoriteToys()
	...
	public void testFetchEagerAndFilter() throws Exception {
		...
		title.getVariations().put(Locale.FRENCH.getLanguage(),
				"title en francais");
		test.setTitle(title);
		s.save(test);
		...
		TestCourse t = (TestCourse) s.get(TestCourse.class,
				test.getTestCourseId());
	...
	public void testDefaultValueColumnForBasic() throws Exception {
		...
		isDefaultValueCollectionColumnPresent(Boy.class.getName(),
				"preferredNames");
		isCollectionColumnPresent(Boy.class.getName(), "nickNames", "element");
		isDefaultValueCollectionColumnPresent(Boy.class.getName(),
				"scorePerPreferredName");
	}
	...
	private void isLegacyValueCollectionColumnPresent(String collectionHolder,
			String propertyName) {

	}

	private void isDefaultValueCollectionColumnPresent(String collectionOwner,
			String propertyName) {
		isCollectionColumnPresent(collectionOwner, propertyName, propertyName);
	}

	private void isCollectionColumnPresent(String collectionOwner,
			String propertyName, String columnName) {
		final Collection collection = getCfg().getCollectionMapping(
				collectionOwner + "." + propertyName);
		final Iterator columnIterator = collection.getCollectionTable()
				.getColumnIterator();
	...
	protected Class[] getAnnotatedClasses() {
		return new Class[] { Boy.class, Country.class, TestCourse.class,
				Matrix.class };
	}
}
