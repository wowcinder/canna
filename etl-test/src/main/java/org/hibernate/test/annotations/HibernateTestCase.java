...
public abstract class HibernateTestCase extends TestCase {
	...
	public static final Logger log = LoggerFactory
			.getLogger(HibernateTestCase.class);
	...
	public void runBare() throws Throwable {
		...
		System.out.println(Environment.getProperties().getProperty( Environment.DIALECT ));
		final Skip skip = determineSkipByDialect(Dialect.getDialect(),
				runMethod);
		if (skip != null) {
			reportSkip(skip);
			return;
	...
	protected final Skip determineSkipByDialect(Dialect dialect,
			Method runMethod) throws Exception {
		...
		RequiresDialect requiresDialectAnn = locateAnnotation(
				RequiresDialect.class, runMethod);
		if (requiresDialectAnn != null) {
			for (Class<? extends Dialect> dialectClass : requiresDialectAnn
					.value()) {
		...
		if (requiresDialectFeatureAnn != null) {
			Class<? extends DialectChecks> checkClass = requiresDialectFeatureAnn
					.value();
	...
	protected <T extends Annotation> T locateAnnotation(
			Class<T> annotationClass, Method runMethod) {
